...
| POST   | `/api/businesses`             | Yes            | Create a business, owned by the caller — `400` if `name` is blank, `financialYearEnd` is missing, or `leadTimeDays` is present but outside 1–90. `leadTimeDays` (issue #53, how many days ahead of a deadline to send its reminder) is optional and defaults to `14` if omitted. Accepts an optional `Idempotency-Key` header (any client-generated string, typically a UUID); resending the same key returns the original business instead of creating a duplicate |
| GET    | `/api/businesses`             | Yes            | List the caller's own businesses, paginated (issue #49) — see "Pagination" below |
| PUT    | `/api/businesses/{id}`        | Yes            | Update name/financialYearEnd/gstRegistered/leadTimeDays — same `400` validation as create, `404` if it doesn't exist or isn't yours. Omitting `leadTimeDays` leaves the business's current value unchanged rather than resetting it to `14` |
