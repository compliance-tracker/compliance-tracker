...
**Recently closed (PRs #68/#69/#70/#71)**: #66 (critical IDOR in `createBusiness` — client-supplied `id` let any user overwrite another user's business), #67 (401 masking legitimate 400/404 errors on `/error`), #27 (ACRA Annual Return deadline not recurring annually), #24 (WorkPass CRUD — the third of three advertised obligations is now actually reachable end to end, not just implemented in `RuleEngine`).
